...
Further, we define $\vec w = \big(w(1),\ldots,w(n)\big)^T$ as the vector of expected rewards.
...
At the beginning of the game, the learner is given a family of feasible sets $\M\subseteq 2^{[n]}$ which corresponds to some combinatorial problem. 
...
We will discuss this extension in Section~X.
...
At each round $t$, \Algorithm accesses the  oracle twice. 
The first access to the oracle computes the set $M_t=\argmax_{M\in \M} \bar w_t(M)$.
Notice that $M_t$ is the ``best'' set according to the empirical means $\vec {\bar w}_t$.
...
Intuitively, the adjusted expectation vector $\vec {\tilde w}_t$ penalizes arms belonging to $M_t$ and encourages exploring arms out of $M_t$.
The algorithm then calls the oracle using the adjusted expectation vector $\vec {\tilde w}_t$ as input, which returns another set $\tilde M_t = \argmax_{M\in \M} \tilde w_t(M)$.
The \Algorithm algorithm stops if $\tilde w_t(\tilde M_t) = \tilde w_t(\tilde M_t)$ and outputs $\out=M_t$.
...
We illustrate these  exchanges classes in \autoref{fig:exchange}.
...
\end{subfigure}
\caption{
Elements of exchange classes $\B_{\MultiIdent}$, $\B_{\Matroid}$, $\B_{\Match}$ and $\B_{\Path}$ for running examples.
(In \autoref{fig:exchange:matroid}, we use spanning tree as a specific instance for the \Matroid problem.)
}
\label{fig:exchange}
\end{figure}

\textbf{Main result.} 
Our main result is a problem-dependent sample complexity bound of \Algorithm algorithm.
...
$$
T \le O\left(R^2\rank(\B)^2\mathbf H\log\left(R^2\rank(\B)^2\mathbf H n/\delta\right)\right),
$$
...
Therefore, the sample complexity bound of our algorithm for the \MultiIdent problem is $O\big(\mathbf H \log(n\delta^{-1} \mathbf H)\big)$.
...
In this section, we establish a problem-dependent lower bound on the sample complexity of the \Problem problem. 
...
We show that, for any combinatorial problem $\M$ and any expected rewards $\vec w$, any $\delta$-correct algorithm $\mathbb A$ must use at least $\tilde\Omega\left(\mathbf H\right)$ samples in expectation.
\begin{theorem}
...
$$
\E[T] \ge \frac{1}{16} \mathbf H \log(\frac{1}{4\delta}),
$$
where 
...
We can extend the \Algorithm algorithm to the fixed budget setting through two simple modifications: (1) requiring \Algorithm to terminate after $T$ rounds; and (2) using a different construction of confidence intervals.
The first modification ensures that \Algorithm use at most $T$ samples, which meets the requirement of the fixed budget setting.
And the second modification guarantees that the confidence intervals are valid for all arms in $T$ rounds.
...
We denote the modified algorithm as \AlgorithmPAC.
In the following theorem, we show that \AlgorithmPAC is indeed an $(\epsilon,\delta)$-PAC algorithm and has sample complexity similar to Theorem~\ref{theorem:main}.
\begin{theorem}
Use the same notations as in Theorem~\ref{theorem:main}.
...
\label{eq:pac}
\end{equation}
where $K=\max_{M\in \M} |M|$ is the size of largest feasible solution.
\end{theorem}
