...
%settle a conjecture on the lower bounds.
...
Formally, if $X$ is a random variable drawn from to $\Rew_e$ for some $e\in[n]$, then, for all $t\in \RR$, one has $\mathbb E\big[\exp(tX-t\mathbb E[X])\big] \le \exp(R^2t^2/2)$.
...
Let $M_*=\argmax_{M\in \M} w(M)$ denote the optimal set belonging to the decision class $\M$ which maximizes the sum of expected reward\footnote{We define $v(S)\triangleq \sum_{i\in S} v(i)$ for any vector $\vec v\in\RR^{n}$ and any set $S \subseteq[n]$. In addition, for convenience, we will assume that $M_*$ is unique.}.
...
Otherwise, \Algorithm pulls the arm belonging to the symmetric difference $(\tilde M_t \del M_t) \cup (M_t \del \tilde M_t)$ between $M_t$ and $\tilde M_t$ which has the largest confidence radius (intuitively the largest uncertainty).
...
Suppose that, for each arm $e\in [n]$, the reward distribution $\Rew_e$ is given by $\Rew_e=\mathcal N(w(e),1)$, where we let $\mathcal N(\mu, \sigma^2)$ denote the Gaussian distribution with mean $\mu$ and variance $\sigma^2$. 
...
Theorem~\ref{theorem:lower-bound} resolves the  conjecture of \citet{bubeck2013multiple} that the lower bounds of sample complexity of \MultiIdent and \MultiBandit problem are $\Omega\big(\mathbf H\log(1/\delta)\big)$, for the case of Gaussian reward distributions on arms.
...
Our lower bound also suggests that the dependency on $\mathbf H$ of the sample complexity of \Algorithm cannot be improven up to logarithmic factors.
...
\end{restatable}
One can verify that $\mathbf H_2$ is equivalent to $\mathbf H$ up to a logarithmic factor, i.e., $\mathbf H_2 \le \mathbf H \le \log(2n) \mathbf H_2$ (see \citep{audibert2010best}).
...
\wei{For the above lemma, should we also add that under the lemma's condition, we also have
	$M_1= M_2 \ominus b$? I think the proof of Lemma 10 uses this fact.}
...
Let $M' \in \M$ be an arbitrary member of the decision class.
We have
$$
...
\begin{align*}
\Pr[S>\epsilon] \le \inf_{t>0} \exp(nR^2t^2/2-t\epsilon)  = \exp(-\epsilon^2 / 2nR^2) = \exp(-n\lambda^2/2R^2).
\end{align*}
Similarly, one can show that $\Pr[S<-\epsilon] \le \exp(-n\lambda^2 / 2R^2)$.
...
\wei{In my other papers, I always use "in round $t$" or "at round $t$". I think "in round $t$" is
	a bit better than "at round $t$", while "at time $t$" is a good term if we refer time instead of 
	round. But "on round $t$" is a bit weird to me. Please check all occurrences use the expression
	consistently.}
\end{proof}
...
First we construct a gadget $M_t'$ as $M_t' = \tilde M_t \ominus c = \tilde M_t \del c_+ \oplus c_-$
\wei{Should this be $\tilde M_t \del c_+ \cup c_-$?}
...
\wei{I think here we need the result that $M_t'\oplus c = \tilde{M}_t$. See my comment after Lemma 1
	about adding such a fact in the lemma.}
...
\wei{It may be better to say that equality in Eq.\eqref{eq:u-c-1-0} is due to our assumption
	$e=p_t$.}
...
\wei{I think it is better to write $\vec \chi_c \in \{-1,0,1\}^n$, since otherwise, the following
equality should be replaced by $\le$.}
\begin{align}
...
\wei{Do we need this fancy inequality? It looks like an easy fact by the special structure
	of $\vec\chi_c$.}
...
\wei{Again, do we need this fancy inequality?}
...
Applying Lemma~\ref{lemma:tilde} by setting $M' = \tilde M_t$ and using the fact that $\tilde w_t(\tilde M_t) \ge \tilde w_t(M_t' \oplus b)$ (since $\tilde w_t(\tilde M_t) = \max_{M\in \M} \tilde w_t(M)$, \wei{This is a repetition, since the explanation of Eq.\eqref{eq:exk-tmp-5} below
	repeats this fact.}
we have 
\begin{align}
...
\wei{I do not understand why Eq.~\eqref{eq:u-c-1-8} and Eq.~\eqref{eq:u-c-1-9} have 
strict inequality sign $<$ instead of $\le$. This is a key issue since we do need a strict
inequality to reach a contradiction.}
...
\end{align*}
\wei{The last $ w_t(M_t)$ should be ${\bar w}_t(M_t)$, right?}
...
\textbf{Notations.} In this section, we will use a notation of ``next-to-optimal set'' defined as follows. 
...
Define $t = \frac{1}{16 d^2}\log(1/\theta)$.
...
and Eq.~\eqref{eq:r64-t-4} follows from the definition of $t$.
Next we have
\begin{align}
...
This can be proven by Markov inequality as follows.
\begin{align*}
...
\end{equation}
By definition of $H_1$ and $H_0$, we see that $w_1(e) = w_0(e)\pm \Delta_e$ (where the sign depends on whether $e\in M_*$).
Therefore, when event $\mathcal S$ occurs, it easy to verify that we can apply Lemma~\ref{lemma:lower-bound-likelihood-gaussian} (by setting $d=w_1(e)-w_0(e)$, $T=T_e$ and $s_i=w_0(e)$ for all $i$).
Hence, by Lemma~\ref{lemma:lower-bound-likelihood-gaussian} and Eq.~\eqref{eq:r64-1}, we have, on event $\mathcal S$,
\begin{equation}
...
\textbf{Step (3): The loss of likelihood.} Similar to the proof of Theorem~\ref{theorem:lower-bound}, we let $L_l$ denote the likelihood function under hypothesis $H_l$ for some $l\in\{0,1\}$. 
...
Since, for all $i \in [T_{b_-}]$, we have $w_1(I_i)-w_0(I_i) = \frac{2\Delta_e}{|b_-|}$, we can apply Lemma~\ref{lemma:lower-bound-likelihood-gaussian} here.
Therefore, on event $\mathcal S$, we have
$$
\frac{L_1(W)}{L_0(W)} \ge \theta.
$$

The rest of the proof is identical to Step (3) in the proof of Theorem~\ref{theorem:lower-bound}, and one can show that $\Pr_1[\mathcal B] \ge \delta$, which means the probability of error of algorithm~$\mathbb A$ is at least $\delta$. 
...
Now we are ready to prove \Cref{fact:matroid}.
