...
In addition, applying our results back to the problems of top-$K$ arms identification and multiple bandit best arms identification, we recover the best available upper bounds up to constant factors and settle a conjecture on the lower bounds.
...
We also notice that our result resolves the conjecture of \citet{bubeck2013multiple} on the problem-dependent sample complexity lower bounds of \MultiIdent and \MultiBandit problems.
...
This suggests that exchange classes may be useful for the analysis of similar problems. 
...
Formally, if $X$ is a random variable drawn according to $\Rew_e$ for some $e\in[n]$, then, for all $t\in \RR$, one has $\mathbb E\big[\exp(tX-t\mathbb E[X])\big] \le \exp(R^2t^2/2)$.
...
Theorem~\ref{theorem:lower-bound} settles the  conjecture of \citet{bubeck2013multiple} that the lower bounds of sample complexity of \MultiIdent and \MultiBandit problem are $\Omega\big(\mathbf H\log(1/\delta)\big)$.
...
2\exp\left(-\frac{nt^2}{2R^2}\right).
$$
\end{lemma}
...
Given any $d \in \RR$ and $\theta \in (0,1)$.
...
\begin{align}
	\exp\left(-\sum_{i=1}^T v_id \right) & \ge \exp\left(-\left|\sum_{i=1}^T v_i \right| \cdot |d| \right)\nonumber\\
																			 & \ge \exp\left(-\sqrt{t\log(1/\theta)} d) \label{eq:r64-t-3}\\
...
\subsection{An exchange set size dependent lower bound}
\label{section:lowerbound-evidence}
We show that, for any arm $e\in [n]$, there exists an exchange set $b=(b_+,b_-)$ which contains $e$ such that a $\delta$-correct algorithm must spend
...
In the rest of the proof, we focus on claim \textbf{(a)}; the claim \textbf{(b)} can proved using an almost identical similar argument.
