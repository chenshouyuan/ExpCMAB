...
Then, we show that, suppose arm $e$ has a gap $\Delta_e$ larger than the ``reference'' gap $\Delta_{(t-n+1)}$ of phase $t$, 
\wei{I think this is a typo, it should be $\Delta_{(n-t+1)}$. Same typo occurs in the lemma statement below.}
...
It shows that if $\Delta_e \ge \Delta_{(t-n+1)}$ for some arm $e$, then the empirical gap of arm $e$,
	$\bar w_t(M_t) - \bar w_t(\tilde M_{t,e})$, is greater than $\frac23 \Delta_{(t-n+1})$.
\wei{Typo again, should be $\Delta_{(n-t+1)}$.}
\begin{lemma}
