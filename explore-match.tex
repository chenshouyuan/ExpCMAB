...
\textbf{Setup.} 
Our task is to identify the optimal routing tree  that has the lowest expected latency from a networking system in an exploration procedure, in which one can obtain noisy measurements of latencies between different nodes.
...
For each network, we model the latency $X(e)$ of edge $e$ to be the sum of the given average latency $l(e)$ and an additive random noise $\mathcal N(0, 1)$.
