...
The learner is also given a collection of sets of arms called \emph{decision class}, which
	satisfy certain combinatorial constraints. 
...
The objective is when the exploration period ends, the learner output a member of the decision
...
A clear challenge is that, instead of solving each \Problem task in an ad-hoc way, one requires a unified algorithm and analysis that supports different decision classes.
...
Our analysis relies on a novel combinatorial construction called \emph{exchange class} which we believe may be of independent interest for other combinatorial optimization problems. 
...
We also notice that our result resolves the open conjectures of \citet{kalyanakrishnan2012pac} and \citet{bubeck2013multiple} on the sample complexity lower bounds of \MultiIdent and \MultiBandit problems.
...
Furthermore, we will use the convention that $\max_{M\in \emptyset} f(M) = -\infty$ for any real-valued function $f$.
...
It is well-known that the family of $R$-sub-Gaussian tail distributions encompasses all distributions that are supported on $[0,R]$ as well as Gaussian distributions with variance $R^2$ (cf. \citep{pollard2000asymptopia}).


We define a \emph{decision class} $\M \subseteq 2^{[n]}$ as a collection of sets of arms.
Let $M_*=\argmax_{M\in \M} w(M)$ denote the optimal set belonging to the decision class $\M$ which maximizes the sum of expected reward\footnote{For convenience, we will assume that $M_*$ is unique. We also assume that $\M$ is non-empty.}. 
...
En route to our sample complexity bound, we introduce the notions of exchange class and widths, which characterize the certain exchange properties of combinatorial structures.
...
We will discuss these extensions in the supplementary material (Section~\ref{section:extensions}).
...
A maximization oracle takes a weight vector $\vec v \in \RR^{n}$ as input and finds an optimal set within a given decision class $\M$ with respect to the weight vector $\vec v$.
...
\textbf{Algorithm.} 
Now we describe the details of \Algorithm.
%
...
We note that \Algorithm generalizes and unifies several different fixed confidence algorithms dedicated to the \MultiIdent and \MultiBandit problems in the literature \citep{kalyanakrishnan2012pac,gabillon2012best}.
...
Therefore, to formally state our result, we need to introduce several notions to capture these structural properties.
...
These patches are later used by our analysis to build gadgets that interpolate between different members of the decision class and serve to bridge different key quantities.
...
In our analysis, the key quantity associated with the exchange class is called \emph{width}, which is defined as the size of largest exchange set as follows
\begin{equation}
\label{eq:width}
...
Next we consider the problem of identifying the maximum matching or the problem of finding the shortest path in a setting where arms correspond to edges.
...
Then we have $\rank(\M_{\Matroid(T)}) \le 2$ (see \Cref{fact:matroid}) and the sample complexity of \Algorithm for $\M_{\Matroid(T)}$ is $O\big(\mathbf H \log(\mathbf H/\delta)\big)$.
\end{example}
In our last example, we see that $\M_{\Matroid(T)}$ is a general type of decision class which encompasses many pure exploration problems including \MultiIdent and \MultiBandit as special cases, where \MultiIdent corresponds to uniform matroids of rank $K$ and \MultiBandit corresponds to partition matroids.
It is easy to see that $\M_{\Matroid(T)}$ also covers the decision class that contains all spanning trees of a graph.
On the other hand, it is well-known that the family of matchings and paths cannot be formulated as matroids since they are matroid intersections \citep{oxley2006matroid}.
...
\end{cases}
$$
where $\M_{A,B}=\{M\in \M \mid A\subseteq M, B\cap M = \emptyset\}$ and $\bot$ is a null symbol.
...
We note that \AlgorithmBud can be considered as a generalization of the two versions of SAR algorithm due to \citet{bubeck2013multiple}, which are designed specifically for the \MultiIdent and \MultiBandit problems respectively.
...
In the following theorem, we bound that the probability of error of the \AlgorithmBud algorithm.
...
\end{restatable}
It is well-known that $\mathbf H_2$ is equivalent to $\mathbf H$ up to a logarithmic factor: $\mathbf H_2 \le \mathbf H \le \log(2n) \mathbf H_2$ (see \citep{audibert2010best}).
...
For each round $t$, we define vector $\vec\rad_t = \big(\rad_t(1),\ldots,\rad_t(n)\big)^T \in [0, +\infty)^n$ and recall that $\vec {\bar w}_t\in \RR^n$ is the empirical mean rewards of arms up to round $t$.
...
Let us consider the set $M_1 = \argmax_{M': M'\in \M \wedge e\in M'} w(M')$ \tian{change to $\argmax_{M' \in \M: e\in M'} w(M')$ for consistency}.
...
Consider the set $M_3 = \argmax_{M': M'\in \M \wedge e\not \in M'} w(M')$ \tian{similarly, change to $\argmax_{M'\in \M: e\not \in M'} w(M')$}.
...
$$
\tian{Change notation from $\tilde{\cal X}$ to $\hat{\cal X}$ to make the difference more clear.}
\label{lemma:tilde}
\end{lemma}

\begin{proof}
We begin with proving the first part.
\tian{From Line 5-8 in Algorithm~\ref{algo:pac}, we have 
$\tilde w_t(e) = \begin{cases} 
				\bar w_t(e)-\rad_t(e) & e \in M_t \\
				\bar w_t(e)+\rad_t(e) & e \not\in M_t
			\end{cases}$.}
...
$$
\tian{I find $\rad_t(e) = R \sqrt{\frac{\log \frac{4nt^2}{\delta}}{2 T_t(e)} }$ is enough, because substituting $t$ with $\rad_t(e)$ in Hoeffding inequality we have:
\begin{align*}
\Pr\left[ \big|\bar w_t(e)-w(e) \big| \ge R\sqrt{\frac{\log\left(\frac{4n t^2}\delta\right)}{2 T_t(e)}} \right] 
	\le 2 \exp\left\{ - \frac{2 T_t(e)}{R^2} \cdot \left(R \sqrt{ \frac{\log \frac{4nt^2}{\delta}}{2 T_t(e)}} \right)^2 \right\} \\
	= 2 \exp\left\{ - \log \frac{4nt^2}{\delta} \right\} \\
	\le \frac{\delta}{2nt^2}.
\end{align*}
}
...
First we construct a gadget $M_t'$ as $M_t' = \tilde M_t \ominus c$ \tian{add $ = (\tilde M_t \setminus c_-) \cup c_+$, then it will be more intuitive.}
	and recall that $M_t' \in \M$ due to the definition of exchange class.
...
Applying Lemma~\ref{lemma:tilde} by setting $M' = M_t' \oplus b$ \tian{I think you replace $M'$ in Lemma~\ref{lemma:tilde} with $M' = \tilde M_t$}
and using the fact that $\tilde w_t(\tilde M_t) \ge \tilde w_t(M_t' \oplus b)$ \tian{in Algorithm~\ref{algo:pac}}, we have 
\begin{align}
...
\tian{(Index error): Eq.~\eqref{eq:u-c-1-9} holds since Eq.~\eqref{eq:u-c-1-0-1} with
	$\inn{\vec \rad_t, \vec \chi_c} \leq \nor{\vec \rad_t \odot |\vec \chi_c|}_\infty \| \chi_c \|_1$ }.
...
\label{eq:u-c-2-0}
\end{equation}
\tian{I think it is $< \frac{\Delta_e}{3 \rank(\B)}$, but I don't know why there are $\rank(\B)$ and $\rank(\M)$ for they are exactly the same. 
Maybe it is better to omit one to reduce the possibility of misunderstanding.}
...
& \ge \inn{\vec w, \vec \chi_b} - 2\inn{\vec \rad_t\odot \vec d, \vec \chi_b} \tian{\text{it is } + 2\inn{\vec \rad_t\odot \vec d, \vec \chi_b}} \label{eq:u-c-2-3} \\
...
For the reader's convenience, we first restate Theorem~\ref{theorem:main} as follows.
\mainresult*
...
We can extend the \Algorithm algorithm to the fixed budget setting by using two simple modifications: (1) requiring \Algorithm to terminate after $T$ rounds; and (2) using a different construction of confidence intervals.
...
In fact, the only modification needed is to change the stopping condition from $\tilde w_t(\tilde M_t) = \tilde w_t(M_t)$ to $w(\tilde M_t)-w(M_t) \le \epsilon$ on line~\ref{algo:step:D} of Algorithm~\ref{algo:pac}.
...
$\rad_{t_e}(e) \ge \min\{\frac{\Delta_e}{3\rank(\B)},\frac{\epsilon}{2K}\}$. \tian{it is $\max\{\frac{\Delta_e}{3\rank(\B)},\frac{\epsilon}{2K}\}$.}
Then, by the construction of $\rad_{t_e}(e)$, we have
\begin{equation}
 \tian{\text{it is } \max\{\frac{\Delta_e}{3\rank(\B)},\frac{\epsilon}{2K}\} }
...
Fix an arm $e\in [n]$. We now focus on proving Eq.~\eqref{eq:lower-each} by contradiction. \tian{Assume $\E[T_e] <  \frac{1}{16\Delta_e^2}\log(1/4\delta)$, and consider ...}
...
\Pr_0[T_e > 4t_e^*] &\le \frac{\E_0[T_e]}{4t_e^*} \\
					  \tian{\text{change this $=$ to $<$}} &= \frac{t_e^*}{4t_e^*} = \frac14.
\end{align*}
...
\tian{Note that $H_1$ and $H_0$ only differs in $\Rew_e$ and the rest distributions are the same.}
Hence, we have
\begin{align}
...
In this case, we know that the mean reward of arm $e$ under $H_1$ is $w(e)-2\Delta_e$.
...
\end{align*}
Now we have proved that, if $\E_0[T_e] < t_e^*$, then $\Pr_1[\mathcal B]>\delta$.
...
\tian{We will prove it by contradiction, thus assume the opposite, that is, $\E_0[T_{b_-}] < t_{b_-}^*$.}
...
\end{align}
This means that, if $\E_0[T_{b_-}] < t_{b_-}^*$, then, under $H_1$, the probability of algorithm $\mathbb A$ returning $M_*$ as output is at least $\delta$. 
...
\tian{We will prove it by contradiction, thus assume the opposite, i.e., 
$\E_0[T_{b_+}] < t_{b_+}^*$.} The arguments used in this part are similar to that of Part (1). 
...
Then, we show that if $\E_0[T_{b_+}] < t_{b_+}^*$, then $\Pr_2[\mathcal B] \ge \delta$.
...
Therefore, we see that if $\E_0[T_{b_+}] < t_{b_+}^*$, then, under $H_2$, the probability of algorithm $\mathbb A$ returning $M_*$ as output is at least $\delta$, which is not the optimal set under $H_2$. 
This contradicts to the assumption that algorithm $\mathbb A$ is a $\delta$-correct algorithm. 
...
Notice that the arm $i$ has been pulled for $\tilde T_t$ times during phases $1,\ldots, t$. 
Therefore,
...
\tian{and Eq.~\eqref{eq:settech-3} holds because $a\cap A_t = \emptyset$, which is derived from $a\subseteq M\del M_*$ and $A_t \subseteq M_*$}
...
At the beginning of phase $t$, there are only $t-1$ inactive arms $|A_t \cup B_t| = t-1$. 
...
\tian{
I find Strong basis exchange property is sometimes assumed to be an axiom, when one studies a special kind of matroids,
such as matroids defined by rank function. So I am not sure whether the strong basis exchange is a standard property.

For the rest cases, one similar lemma I find is in the book edited by Neil White,
which seems to be a better source than a lecture note.
This lemma looks suitable, and you may check whether it works as well.

\begin{lemma}[Proposition~4.1.1: Symmetric basis-exchange axiom]
Let $\mathcal A$ the set of all bases of a matroid $T=(E,\mathcal I)$. 
Let $A_1, A_2 \in \mathcal A$ be two members of $\mathcal A$.
Then for all $x \in A_1$, there exists $y \in A_2$,
such that $A_1 \del \{x\} \cup \{y\} \in \mathcal A$ and $A_2 \del\{y\}\cup\{x\} \in \mathcal A$.
\end{lemma}
And in Theorem~4.3.1, $x, y$ are generalized to two subsets $X, Y$.

\begin{lemma}[Theorem~4.3.1: Multiple basis-exchange theorem]
Let $\mathcal A$ the set of all bases of a matroid $T=(E,\mathcal I)$. 
Let $A_1, A_2 \in \mathcal A$ be two members of $\mathcal A$.
Then for all $X \subseteq A_1$, there exists $Y \subseteq A_2$,
such that $A_1 \del X \cup Y \in \mathcal A$ and $A_2 \del Y \cup X \in \mathcal A$.
\end{lemma}

%Here is the bib:
%@book{white1986theory,
...
The idea of the reduction is simple: one can impose the negative constrains $B$ by setting the corresponding weights to be sufficiently small; and one can impose the positive constrains $A$ by setting the corresponding weights sufficiently large.
The reduction method is shown in Algorithm~\ref{algo:reduction}.
The correctness of the reduction is proved in Fact~\ref{fact:coracle}.
...
Therefore, Fact~\ref{fact:coracle} shows that maximization oracles are equivalent to constrained oracles up to a transformation on the weight vector. 
...
Since $\vec w_2$ and $\vec w_1$ coincide on entries of $[n]\del B$, we have
\begin{equation}
\label{eq:rdc1}
w_2(M) = w_1(M).
\end{equation}
...
$$
w_2(M_0) = w_1(M_0) = w_1(A)+w_1(M\del A) \tian{w_1(M_0 \del A)} = 3|A| L_1+w(M_0\del A) \ge 3|A| L_1-L_1 = (3|A|-1)L_1.
$$
