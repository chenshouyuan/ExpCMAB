...
\newcommand{\MultiIdent}{\textbf{Multiple Identification}}
\newcommand{\Match}{\textbf{Matching}}
\newcommand{\Path}{\textbf{Shortest Path}}
...
After the game stops, the learner is asked to output a set of arms $\out \in \M$ which approximately maximizes the sum of expected weight, i.e. $w(M_*) - w(\out) \le \epsilon$, where we denote $M_*=\argmax_{M\in \M} w(M)$ to be the optimal set of arms.
...
We consider the following concrete tasks as the exemplar applications of our model and learning algorithm.
...
We begin with defining a natural complexity measure of the \Problem problem. 
...
\begin{theorem}
For each arm $i\in [n]$, assume that its reward distribution of arm $i$ is a Gaussian distribution with mean $p_i$ and variance 1.
...
For each $i\in [n]$, assume that the reward distribution of arm $i$ a Gaussian distribution with mean $p_i$ and variance 1.
...
\begin{theorem}
Assume that, for each arm $i\in [n]$, its reward distribution $\Rew_i$ is a Gaussian distribution with mean $p_i$ and variance 1.
