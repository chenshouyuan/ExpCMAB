...
\wei{I changed the general index of an arm from $i$ to $e$, following the common convention used in the paper. I also did this for the
algorithm pseudocode. Please check other places to try to keep this convention.}
...
Denote $\bar w^{(s)}(e)$ as the empirical mean of $\Rew_e$ from $s$ samples, and by definition 
$\bar w_t(e) = \bar w^{(T_t(e))}(e)$ always holds.
\begin{align}
\Pr\left[ \big|\bar w_{t}(e)-w(e) \big| \ge R\sqrt{\frac{2\log\left(\frac{4n t^3}\delta\right)}{T_t(e)}} \right] 
&= \sum_{s=1}^{t-1} \Pr\left[ \big|\bar w^{(s)}(e)-w(e) \big| \ge R\sqrt{\frac{2\log\left(\frac{4n t^3}\delta\right)}{s}}, T_t(e) = s\right]  \nonumber \\
&\le \sum^{t-1}_{s=1} \frac{\delta}{2nt^3} \label{eq:ci-1} \\
&\le \frac{\delta}{2nt^2}, \nonumber
\end{align}
where Eq.~\eqref{eq:ci-1} is derived by Hoeffding's Inequality (Lemma~\ref{lemma:hoeffeding}).

{\em Note: } The above is a quick fix. Wei's analysis tightens the confidence interval from $\sqrt{2 \log\left(\frac{4n t^3}\delta\right)/ T_t(e)}$ to $\sqrt{3 \log\left(\frac{4n t^3}\delta\right)/ 2 T_t(e)}$.
}
...
\wei{The last inequality is due to the assumption of the lemma, not that $\xi_t$ occurs, right? Please check.
}
...
\wei{Lemma~\ref{lemma:vector-technical} has been applied two lines ago, not here at Eq.~\eqref{eq:u-c-2-3.5}.}
...
\tian{
Since $w_t(e)$ is a random variable, similarly we bound probability as follows
\begin{align*}
\Pr\left[ \big|\bar w_t(e)-w(e) \big| \ge \rad_t(e) \right]
	&= \sum_{s=1}^{t-1} \Pr\left[ \big|\bar w^{(s)}(e)-w(e) \big| \ge R\sqrt{\frac{\alpha}{s}}, T_t(e) = s \right] \\
	&\le \sum_{s=1}^{t-1}  2 \exp(-\alpha/2)
	\le 2 t \exp(-\alpha/2).
\end{align*}
The above inequality will bring in another $T$ in the probability.
}
...
By Lemma~\ref{lemma:fbudget:ci}, \tian{Revise the constant when we apply this lemma} we know that the event $\tau \triangleq \bigcap_{t=1}^T \tau_t$ occurs with probability at least
