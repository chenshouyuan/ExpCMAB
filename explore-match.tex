...
For more than fifty years, multi-armed bandit (MAB) has been a predominant approach for modeling the tradeoff between exploration and exploitation in decision-making problems. 
...
Our algorithm \Algorithm does not need to know anything about the details of the decision class's definition, as long as it has access to the decision class through a maximization oracle.
...
Now we  define the \Problem problem.
...
Following standard assumptions of stochastic MABs, we assume that all reward distributions are $R$-sub-Gaussian. 
Formally, if $X$ is a random variable drawn according to $\Rew_e$, then, for all $t\in \RR$, $\mathbb E\big[\exp(tX-t\mathbb E[X])\big] \le \exp(R^2t^2/2)$ and $\mathbb E\big[\exp(t\mathbb E[X]-tX)\big] \le \exp(R^2t^2/2)$.
...
\textbf{Examples of decision classes.} 
%The formulation of the \Problem problem covers many online learning tasks.
The formulation of \Problem allows arbitrary decision classes $\M$.
In this paper, we consider the following types of decision classes as our running examples.
%By the definition of \Problem, these decision classes lead to different pure exploration problems where the object of interest $M_*$ has different combinatorial structures.
\begin{itemize}
\item \textbf{Top $K$ arms} (\MultiIdent). For any $K\in[n]$, 
consider of the problem finding the top $K$ arms with the largest expected reward. 
This problem can be modeled in the \Problem framework by using the top-$K$ decision class $\M_{\MultiIdent(K)}$ which is defined as the collection of all size-$K$ sets, formally $\M_{\MultiIdent(K)}=\{M\subseteq [n] \mid \big|M\big| = K\}$.
\item \textbf{Multi-bandit best arms} (\MultiBandit). Let $\mathcal A=\{A_1,\ldots,A_m\}$ be a partition of $[n]$. 
Consider the problem of identifying the best arms from each group of arms $A_1,\ldots,A_m$.
The multi-bandit decision class associated with $\mathcal A$ is defined as 
$\M_{\MultiBandit(\mathcal A)} = \{M \subseteq [n] \mid \forall i\in [m], |M\cap A_i| = 1\}$.
\item \textbf{Maximum matching} (\Match). 
Let $G(V,E)$ be a bipartite graph with $n$ edges.
%Consider the problem of finding the matching with maximum expected weight. 
%We can model this problem by using the matching decision class as follows.
Let $\sigma\colon E\rightarrow [n]$ be a bijection.
The matching decision class associated with $G$ is defined as $\M_{\Match(G,\sigma)}=\{M \subseteq [n] \mid \sigma^{-1}(M) \text{ is a matching in } G\}$.
%This decision class corresponds to the problem of finding the matching with maximum expected weight. 
		%Let $\mathbb H_{\Match} = \{\forall G,\; \M_{\Match(G)}\}$ denote the family of all matching decision classes.
\item \textbf{Shortest path} (\Path).
Let $G(V,E)$ be a directed acyclic graph where $|E|=n$.
Let $s, t \in V$ be two vertices of $G$.
%The problem of finding the shortest path from $s$ to $t$ can be modeled using the path decision class as follows.
Let $\sigma\colon E\rightarrow [n]$ be a bijection.
The path decision class associated with $G,s,t$ is defined as $\M_{\Path(G,s,t,\sigma)}=\{M \subseteq [n] \mid \sigma^{-1}(M) \text{ is a path from } s\text{ to }t\text{ in } G\}$.
\item \textbf{Maximum-weight basis of matroid} (\Matroid).
...
Then, the matroid decision class associated with $T$ is defined as 
$\M_{\Matroid(T,\sigma)}=\{M\subseteq [n] \mid \sigma^{-1}(M)\text{ is a basis of } T\}$.
\end{itemize}

The last example \Matroid is an abstract type of decision class which encompasses \MultiIdent and \MultiBandit as special cases, where \MultiIdent corresponds to uniform matroids of rank $K$ and \MultiBandit corresponds to partition matroids.
The decision class that contains all spanning trees of a graph is also covered by \Matroid.
On the other hand \Match and \Path cannot be formulated as matroids and in fact these problems are equivalent to matroid intersections (cf. \citep{oxley2006matroid}). 
...
\textbf{Remarks.} By combining \Cref{lemma:example-exchange-class} and \Cref{theorem:main}, we obtain the sample complexity of \Algorithm for our example decision classes. For $\M\in \mathbb H_{\Matroid}$, the sample complexity is $O\big(\mathbf H \log(n\mathbf H/\delta)\big)$. 
And for $\M\in \mathbb H_{\Match}\cup\mathbb H_{\Path}$, the sample complexity is $\tilde O(|V|^2 \mathbf H)$.
...
We have $\rank(\M_{\MultiIdent(K)}) \le 2$ and $\rank(\M_{\MultiBandit(\mathcal A)}) \le 2$
(\Cref{lemma:example-exchange-class} in the appendix) and therefore the sample complexity of \Algorithm for solving \MultiIdent and \MultiBandit is $O\big(\mathbf H \log(n\mathbf H/\delta)\big)$, which recovers previous results \citep{kalyanakrishnan2012pac,gabillon2012best}.
\end{example}

\begin{example}[Matchings and Paths]
Let $G(V,E)$ be a graph with $n$ edges and assume there is a one-to-one mapping between $E$ and $[n]$.
(1) Suppose that $G$ is a bipartite graph. 
Let $\M_{\Match(G)}$ denote the set of all matchings in $G$.
We have $\rank(\M_{\Match(G)}) \le |V|$ (\Cref{lemma:example-exchange-class}).
(2) Suppose that $G$ is a direct acyclic graph and let $s,t\in V$ be two vertices.
Let $\M_{\Path(G,s,t)}$ denote the set of all paths from $s$ to $t$.
We have $\rank(\M_{\Path(G,s,t)}) \le |V|$ (\Cref{lemma:example-exchange-class}).
...
The last example is an abstract type of decision class which encompasses \MultiIdent and \MultiBandit as special cases, where \MultiIdent corresponds to uniform matroids of rank $K$ and \MultiBandit corresponds to partition matroids.
The decision class that contains all spanning trees of a graph is also covered by \Cref{example:matroid} (cf. \citep{oxley2006matroid}).
...
This gap also suggests that the dependency on $\mathbf H$ of the sample complexity of \Algorithm cannot be improved up to logarithmic factors.
Furthermore, we conjecture that the sample complexity lower bound might inherently depend on the size of exchange sets, which is further bounded by $\rank(\M)$. 
...
\end{equation}
Hence we see that $\COracle(\vec v, A, B)$ computes an optimal solution that includes all elements of $A$ while exclude all elements of $B$.
...
In the supplementary, we show that constrained oracles can be easily reduced to maximization oracles. 
...
For the \MultiIdent problem, we see that this matches previous fixed budget algorithm due to \citet{bubeck2013multiple}.
