...
We study {\em combinatorial pure exploration (\Problem)} problem in the stochastic multi-armed bandit setting,
	...
	satisfying certain combinatorial constraints, such as size-$K$ subsets, matchings, spanning trees,
	paths, etc.
A collection of subsets of arms satisfying a combinatorial constraint is referred to as a {\em decision class}.	
Instead of solving each \Problem task in an ad-hoc way, we provide a general framework
	and solutions that work for all decision classes which admit offline maximization oracles.  
...
Although this tradeoff is intrinsic in many tasks, some application domains prefer a dedicated exploration procedure in which the goal is to identify an optimal object among a set of candidates and the reward or loss incurred during exploration is irrelevant.
...
Furthermore, we will use the convention that $\max_{M\in \emptyset} f(M) = -\infty$ and $\argmax_{M\in \emptyset} f(M) = \bot$ for any real-valued function $f$.
...
This naturally generalizes previous definitions of gaps \citep{audibert2010best,NIPS2011_4478,kalyanakrishnan2010efficient,bubeck2013multiple}.
...
Next, we define the \emph{width} of exchange class $\B$ to be the size of largest exchange set as follows
\begin{equation}
\label{eq:width}
\rank(\B) = \max_{(b_+,b_-) \in \B} |b_+|+|b_-|.
\end{equation}
Intuitively an exchange class $\B$ for $\M$ can be seen as a collection of ``patches'' (borrowing concepts from software engineering) such that, for any two different sets $M,M'\in \M$, one can convert from $M$ to $M'$ by applying a series of patches from $\B$, i.e. $M'=M\oplus b_1 \oplus \ldots \oplus b_k$. 
These patches are later used by our analysis to build gadgets that interpolate between different members of the decision class and serve to bridge different quantities.
...
%$\COracle(\vec v, A, B) \in \argmax_{M \in  \M_{a,b}} v(M)$,
%where $\M_{a,b}=\{M\in \M \mid a\subseteq M, b\cap M = \emptyset\}$.
Hence we see that $\COracle(\vec v, A, B)$ returns an optimal set that includes all elements of $A$ while excluding all elements of $B$; and if there is no feasible set (i.e. $\M_{A,B} = \emptyset$), the oracle $\COracle(\vec v, A, B)$ returns the null symbol $\bot$.
...
We show that constrained oracles are equivalent to maximization oracles up to a transformation on the weight vector in the supplementary material.
...
We note that \AlgorithmBud can be considered as a generalization of the SAR algorithm due to \citet{bubeck2013multiple}, which is designed specifically for the \MultiIdent and \MultiBandit problems.
\begin{algorithm}[htbp]
{
\small
\begin{algorithmic}[1]
...
\If{$B\cap M = \emptyset$ and $A\subseteq \out$}
  \State $\out=M$
\Else
  \State $\out=\bot$
\EndIf
