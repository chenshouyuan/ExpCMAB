...
\begin{equation}
\label{eq:cmr-t-1}
T \le 458\mathbf{\tilde H}\log\left(4n\mathbf{\tilde H}/\delta\right)+2n.
\end{equation}
...
\end{equation}
If $C \le 458$, then it is clear that Eq.~\eqref{eq:cmr-t-1} holds.
Suppose, on the contrary, that $C > 458$.
Notice that $T=\sum_{e\in[n]} T(e)$. 
...
\begin{align}
	T &\le n+\sum_{e\in [n]}\frac{18 \rank(\M)^2 R^2}{\Delta_e^2} \log(4nT^3/\delta) \nonumber\\
		...
		&=n+(126+54\log(2C))\log(4n\mathbf{\tilde H}/\delta)\nonumber \\
		&< n+C\log(4n\mathbf{\tilde H}/\delta) \label{eq:cmr-t-5}\\
		&= T,\label{eq:cmr-t-0}
\end{align}
...
Eq.~\eqref{eq:cmr-t-5} follows since $126+54\log(2C) < C$ for all $C>458$;
and Eq.~\eqref{eq:cmr-t-0} is due to Eq.~\eqref{eq:cmr-t-3}.
Hence we see that Eq.~\eqref{eq:cmr-t-0} is a contradiction. 
Therefore we see that $C\le 458$ which means that Eq.~\eqref{eq:cmr-t-1} holds.
...
only a unique set of arms remains to satisfy all constraints of phase $t$, i.e. $|\mathcal M_{A_t,B_t}|=1$ (see Section~\ref{section:csar} for the definitions of $A_t, B_t$ and $\mathcal M_{A_t,B_t}$).
By the design of \AlgorithmBud, this unique element of $\mathcal M_{A_t,B_t}$ will be the output of algorithm.
In this case, the remaining budget for phases $t+1,\ldots, n$ are wasted in the sense that they do not affect the output.
Therefore, in our experiment, we modify \AlgorithmBud to utilize these remaining budget using the following heuristic.
If $|\mathcal M_{A_t,B_t}|=1$ for some $t < n$, we terminate the algorithm and then re-run the algorithm on the remaining budget.
...
We keep this process until the the algorithm terminates normally or the remaining budget is less than 10\% of the original budget.
