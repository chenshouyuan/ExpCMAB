...
\begin{abstract}
We study {\em combinatorial pure exploration (\Problem)} in the stochastic multi-armed bandit setting,
...
Instead of solving each \Problem task in an ad-hoc way, we provide a general framework
	and solutions that work for all decision classes with linear reward functions and exact offline maximization oracles.  
...
\textbf{Notations.} Let $[n]$ denote the set $\{1,\ldots,n\}$. Suppose that $\vec w\in \RR^{n}$ is a vector and $E\subseteq[n]$ is a set. 
Let $w(i)$ denote the $i$-th entry of $\vec w$. 
We also define $w(E) \triangleq  \sum_{i\in E} w(i)$ to be the sum of entries indexed by $E$.
...
It is well-known that all distributions that are supported on $[0,R]$ have $R$-sub-Gaussian tails \citep{pollard2000asymptopia}.
...
\For{$t=n,n+1,\ldots$}
  ...
  \Comment{Break ties arbitrarily}
	...
	\EndFor
\end{algorithmic}
\caption{\Algorithm: Combinatorial Gap based Exploration}
\label{algo:pac}
}
\end{algorithm}
...
Next we state two basic lemmas that help us to convert set-theoretical arguments to linear algebra arguments.
\begin{lemma}
\label{lemma:max}
