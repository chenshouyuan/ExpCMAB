...
In the following, we present their definitions.
...
Then, for any $e \in (M\del M')$, there exists an exchange set $(b_+,b_-)\in \B$ which satisfies $e\in b_-$, $b_+ \subseteq M'$, $b_- \subseteq M$ and $(M\oplus b) \in \M$.
...
Intuitively, for any feasible sets $M$ and $M'$, there exists an ``operation'' $(b_+,b_-)\in \B$ which can transform $M$ one step towards $M'$; this operation generates a new feasible set $M\oplus b$ by removing elements (including $e$) from $M$ and adding elements which belongs to $M'$.
...
And for \Match and \Path problems, we use the graph-theoretical properties of matchings and paths.
Figure~Y illustrates these  exchanges classes.
A detailed proof of Lemma~\ref{lemma:example-exchange-class} is deferred to the supplementary material.


\textbf{Main results.} 
We show that, in the fixed confidence setting, our algorithm returns the optimal set with high probability and uses at most $\tilde O(\rank(\B)^2/\Delta_e^2)$ samples.
\begin{theorem}
Given any $\delta \in (0,1)$, any combinatorial problem $\M \subseteq 2^{[n]}$ and any expected rewards $\vec w \in \RR^{d}$.
Let $\B$ be an exchange class for $\M$ and $\{\Delta_e\}_{e\in [n]}$ be the gaps defined in Eq.~\eqref{eq:define-delta}.
Then, with probability at least $1-\delta$, Algorithm~\ref{algo:pac} returns the optimal set and
$$
T \le O\left(\sum_{e\in [n]} \frac{\rank(\B)^2}{\Delta_e^2} \log\left(\frac{n}{\delta} \sum_{e\in[n]}\frac{\rank(B)^2}{\Delta_e^2} \right)\right),
$$
where $T$ denotes the number of samples used by Algorithm~\ref{algo:pac}.
\label{theorem:main}
\end{theorem}
Theorem~\ref{theorem:main} is highly general in the sense that it provides a sample complexity for any combinatorial problem $\M$.
In addition, notice that $\rank(\B_{\MultiIdent})=O(1)$.
Therefore, our sample complexity bound for the \MultiIdent problem is $O(\sum_e \Delta_e^{-2}\log(n/\delta\sum_{e}\Delta_a^{-2}))$.
This matches the previous best problem-dependent bounds for the \MultiIdent problem.
