...
\junk{The pure exploration problem is about designing strategies that make the best use of the limited budget.
...
\section{Algorithm, Exchange Class and Main Result}
...
En route to our main result, we propose the notions of exchange class and width of combinatorial problems, which characterize the exchange properties of combinatorial structures.
...
When specializing to \MultiIdent problem, our definition resembles the previous definition of gaps due to \citet{kalyanakrishnan2012pac} and \citet{gabillon2012best}.
...
For many problems, there are exchange classes with small widths that correspond to natural combinatorial structures. 
...
Notice that we have $\rank(\M) \le n$ for any $\M$ and therefore the gap is relatively benign.
Ignoring the logarithmic factors, this gap only depends on the underlying combinatorial structure of $\M$.
This suggests that the dependency on $\mathbf H$ of the sample complexity of \Algorithm cannot be improved up to logarithmic factors for general combinatorial problems.
Furthermore, we conjecture that the sample complexity lower bound might intrinsically depend on the size of exchange sets (a quantity which is bounded by $\rank(\M)$). 
In the supplementary material, we provide evidence on this conjecture which is a lower bound on the sample complexity of exploration on exchange sets.
...
\section{Agnostic Fixed Budget Algorithm}
...
The \AlgorithmBud algorithm requires access to a stronger oracle which is referred as \emph{constrained oracle}.
A constrained oracle for $\M$ is a function denoted as $\COracle: \RR^{n}\times 2^{[n]} \times 2^{[n]} \rightarrow \M$.
...
\textbf{\AlgorithmBud algorithm.}
The idea of the \AlgorithmBud algorithm is as follows.
The \AlgorithmBud algorithm divides the budget of $T$ pulls into $n$ phases. 
...
Therefore the major challenge here is to choose a correct arm to accept/reject. 
Clearly, for problems with non-trivial combinatorial structures, one cannot simply accept/reject arms according to their empirical means, since an arm with small expected reward may belong to the optimal solution.
...
\textbf{Analysis.}
We show that the \AlgorithmBud algorithm guarantees a probability of error upper bounded by $\tilde O(\exp(-T\rank(\M)^{-2} \mathbf H^{-1}))$ in the following theorem.
...
Given budget $T > n$, there exists an algorithm (in particular, Algorithm~\ref{algo:budget}) which uses at most $T$ samples and outputs a solution $\out \in \M$ such that
\begin{equation}
...
\end{restatable}
The quantity $\mathbf H_2$ defined in Theorem~\ref{theorem:budget} can be considered as a surrogate of $\mathbf H$ since these two quantities are equivalent up to a logarithmic factor: $\mathbf H_2 \le \mathbf H_1 \le \log(2n) \mathbf H_2$.
For the \MultiIdent problem, we see that this matches previous fixed budget algorithm due to \citet{bubeck2013multiple}.

We see that $\rank(\M)^2$ also appears in the probability of error Eq.~\eqref{eq:fbudget:prob-error} in Theorem~\ref{theorem:budget}, which indicates that $\rank(\M)$ may reflect the inherent hardness associate with a combinatorial problem $\M$ for the \Problem problem.
...
We notice that the \AlgorithmBud algorithm uses solely the empirical means of arms to support its decision making; while our main algorithm \Algorithm also need to use explicitly constructed confidence intervals of the arms.
From this viewpoint, the \AlgorithmBud algorithm is very different from our main algorithm \Algorithm. 
...
\subsection{Analysis of \AlgorithmBud}
...
\subsubsection{Proof of Theorem~\ref{theorem:budget}}
...
\subsection{Fixed Budget Setting (Theorem~\ref{theorem:main-budget})}
...
\subsection{PAC Learning (Theorem~\ref{theorem:pac})}
