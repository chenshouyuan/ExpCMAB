...
The learner is also given a collection of sets of arms called \emph{decision class}, which
	correspond to  certain combinatorial structures.
...
\textbf{Setup.} For each task, we assign the expected reward of arms using the corresponding dataset, and rewards are typical modeled as the sum of the expected reward and an additive zero-mean noise.
In this way, the ground-truth of expected reward $\vec w$ is known in each task.
This allows us to know the ground-truth of the optimal set $M_*$ and the hardness measures $\mathbf H$ in each task.
Furthermore, from this ground-truth, we generate a number instances with different $\mathbf H$ by adjusting the expected reward of each arm $e\in M_*$ with a same amount $c_0>0$.
...
For each task, we use the following evaluation procedure to compare the sample efficiency between \Algorithm,\AlgorithmBud and \Uniform.
...
Our first task is to identify the optimal routing tree from a networking system that has the lowest expected latency in a dedicated exploration procedure, in which one can obtain noisy measurements of latencies between different nodes.
...
We model the latency $X(e)$ of edge $e$ to be the sum of the average latency $w(e)$ and an additive random noise $\mathcal N(0, 1)$.
Then we model the reward of edge $e$ as the negative latency $-X(e)$.
...
From the results, we see that both \Algorithm and \AlgorithmBud are consistently more sample efficient than \Uniform by a large margin (Figure~Z).
