...
For clarity, we define $\mathbf H = \sum_{i\in [n]}\Delta_i^{-2}$.

First, we show that, in the fixed confidence setting, our algorithm returns the optimal set with high probability and uses at most $\tilde O\big(\rank(\B)^2 \mathbf H\big)$ samples.
\begin{theorem}
...
Then, with probability at least $1-\delta$, \Algorithm (Algorithm~\ref{algo:pac}) returns the optimal set $\out=M_*$ and
$$
T \le O\left(\frac{1}{R^2}\rank(\B)^2\mathbf H\log\left(\frac{1}{R^2}\rank(\B)^2\mathbf H\right)\right),
$$
...
$$
as long as $\alpha < 9T\left(B^2\rank(\B)^2 \mathbf H\right)^{-1}$.

\label{theorem:main-budget}
\end{theorem}

Theorem~\ref{theorem:main-budget} suggests that the probability of error of \Algorithm in the fixed budget model is bounded by $O(Tn\exp(-T/(\rank(\B)^2\mathbf H)))$ for certain parameter $\alpha$. 
...
$$
\E[T] \ge \frac{1}{16} \mathbf H \log(1/4\delta),
$$
where 
...
In this section, we prove our main results: Theorem~\ref{theorem:main} and Theorem~\ref{theorem:main-budget}


\textbf{Notations.} 
For our analysis, we need some additional notations.
...
Define 
$$
t_e^* = \left\lceil\frac{275}{R^2}\cdot\frac{\rank(\B)^2}{\Delta_e^2}\log\left(\frac{1}{R^2}\cdot\frac{4n}{\delta}\sum_{i\in[n]}\frac{\rank(\B)^2}{\Delta_i^2}\right)\right\rceil.
$$
...
\begin{align*}
\rad_t(e) &=   R\sqrt{\frac{2\log\left(\frac{4n t^2}\delta\right)}{T_e(t)}} \le
 R\sqrt{\frac{2\log\left(\frac{4n (t_e^*)^2}\delta\right)}{t_e^*}} <  \frac{\Delta_e}{3\rank(\B)}. 	
\end{align*}
...
O\left(\frac{1}{R^2}\sum_{e\in [n]} \frac{\rank(\B)^2}{\Delta_e^2} \log\left(\frac{1}{R^2}\cdot \frac{n}{\delta} \sum_{e\in[n]}\frac{\rank(\B)^2}{\Delta_e^2} \right)\right).
\end{align*}
