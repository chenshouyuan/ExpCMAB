...
Specifically, we assume that there exists an oracle which takes expected reward vector $\vec w = \big(w(1),\ldots,w(n)\big)^T$ as input and returns a set $\Oracle(\vec w) = \argmax_{M\in \M} w(M)$.
...
\begin{theorem}
Given any $\delta \in (0,1)$, any combinatorial problem $\M \subseteq 2^{[n]}$ and any expected rewards $\vec w \in \RR^{n}$.
...
Let $\vec \chi_a \in \{0,1\}^n$ denote the incidence vector of set $a \subseteq [n]$, i.e. $\chi_a(e) = 1$ if and only if $e\in a$.
...
and Eq.~\eqref{eq:l-d-3} follows since $\vec\chi_{M'}-\vec\chi_{M_t}\in \{-1,0,1\}^2$.
...
Using a union bound over all $t>0$ again, we have
\begin{align*}
