...
\wei{It is not immediately clear how to solve the inequality with $T$ on both sides. Can you write more steps here? For all the proofs
so far, one can follow the proofs step by step without doing any computation on a separate paper, but this final step one has to do some
computation and it is not clear how to do it. So to be consistent, I think it is better to provide more detailed steps here.
This is also related to reviewer's comment of providing the explicit bound with constants shown, not the big O bound, so it is worth to
give the derivation.
}
\end{proof}
