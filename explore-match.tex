...
\author{
Shouyuan Chen$^1$\thanks{This work was done when the first two authors were interns at Microsoft Research Asia.}\hspace{1.5em}Tian Lin$^2$\hspace{1.5em}Irwin King$^1$\hspace{1.5em}Michael R. Lyu$^1$\hspace{1.5em}Wei Chen$^3$\\
$^1$The Chinese University of Hong Kong\hspace{1.5em}$^2$Tsinghua University\hspace{1.5em}$^3$Microsoft Research Asia\\
{ $^1$\{sychen,king,lyu\}@cse.cuhk.edu.hk \hspace{1em} 
$^2$lint10@mails.tsinghua.edu.cn\hspace{1em}$^3$weic@microsoft.com}\\
}
\date{\today}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For example, a crowdsourcing application may want to find the best assignment from workers to tasks such that overall productivity of workers is maximized. 
...
At the beginning of the game, the decision class $\M$ is revealed to the learner while the reward distributions $\{\Rew_e\}_{e\in[n]}$ are unknown to her.
Then, the learner plays the game over a sequence of rounds;
in each round $t$, she pulls an arm $p_t\in [n]$ and observes a reward sampled from the associated reward distribution $\Rew_{p_t}$.
...
She need to guarantee that $\Pr[\out = M_*] \ge 1-\delta$ for a given confidence parameter $\delta$.
...
In this setting, her performance is measured by the probability of error.
...
Set $\rad_t(e) = R\sqrt{2\log\left(\frac{4n t^3}\delta\right)/ T_t(e) }$ for all $t > 0$ and $e\in[n]$.
...
T \le O\left(R^2\rank(\M)^2\mathbf H\log\left(nR^2 \mathbf H /\delta\right)\right),
\end{equation}
...
A  detailed analysis and the constructions of exchange classes can be found in the supplementary material (\Cref{section:exchange-proof}).
%Our bounds of width are proven b
%and apply Theorem~\ref{theorem:main} for these classes. 
We begin with the problems of top-$K$ arm identification (\MultiIdent) and multi-bandit best arms identification (\MultiBandit).
...
Note that maximization oracles for these two decision classes are trivially the functions of returning the top $k$ arms or the best arms of each group.
...
Next we consider the problem of identifying the maximum matching and the problem of finding the shortest path (by negating the rewards), in a setting where arms correspond to edges.
...
Let $T=(E,\mathcal I)$ be a finite matroid, where $E$ is a set of size $n$ (called ground set) and $\mathcal I$ is a family of subsets of $E$ (called independent sets) which satisfies the axioms of matroids (see \Cref{footnote:matroid} in \Cref{section:exchange-proof}).
...
Then we have $\rank(\M_{\Matroid(T)}) \le 2$ (derived from strong basis exchange property of matroids; see \Cref{fact:matroid}) and the sample complexity of \Algorithm for $\M_{\Matroid(T)}$ is $O\big(\mathbf H \log(n\mathbf H/\delta)\big)$.
\end{example}
\vspace{-0.5em}
...
The arms that are neither accepted nor rejected are sampled for an equal number of times in the next phase.
...
It is clear that the output of $\COracle(\vec {\bar w}_t, A_t,B_t)$ is independent from the input $\bar w_t(e)$ for any $e\in A_t\cup B_t$.
...
\For{$t=1,\ldots,n$}
	...
	\Comment{set $\bar w_t(e)= 0, \;\forall e\in A_t\cup B_t$}
  ...
  \EndIf
	...
	\EndFor
	\State $p_t \gets \argmax_{e\in[n]\del (A_t\cup B_t)} \bar w_t(M_t) - \bar w_t(\tilde M_{t,e})$
...
Grand Fundamental Research 973 Program of China (No.
2014CB340401 and No. 2014CB340405), the Research Grants
Council of the Hong Kong Special Administrative Region, China
(Project No. CUHK 413212 and CUHK 415113), and Microsoft Research
Asia Regional Seed Fund in Big Data Research (Grant No.
...
$$
\rad_t(e) = R\sqrt{\frac{2\log\left(\frac{4n t^3}\delta\right)}{T_t(e)}},
$$
...
Fix any $t>0$ and $e\in [n]$. 
Note that $\Rew_e$ is a $R$-sub-Gaussian tail distribution with mean $w(e)$ and $\bar w_t(e)$ is the empirical mean of $\Rew_e$ from $T_t(e)$ samples. 
Then, we have
%Using Hoeffding's inequality (Lemma~\ref{lemma:hoeffeding}) and a union bound over the possible values of $T_t(e)$, we obtain
\begin{align}
\Pr\left[ \big|\bar w_t(e)-w(e) \big| \ge R\sqrt{\frac{2\log\left(\frac{4n t^3}\delta\right)}{T_t(e)}} \right] &=
\sum_{s=1}^{t-1} \Pr\left[ \big|\bar w_t(e)-w(e) \big| \ge R\sqrt{\frac{2\log\left(\frac{4n t^3}\delta\right)}{s}}, T_t(e) = s \right]
\label{eq:cmr-3-1}\\
&\le \sum_{s=1}^{t-1} \frac{\delta}{2nt^3}
\label{eq:cmr-3-2}\\
&\le \frac{\delta}{2nt^2},\nonumber
\end{align}
where Eq.~\eqref{eq:cmr-3-1} follows from the fact that $ 1 \le T_t(e) \le t-1$ and
Eq.~\eqref{eq:cmr-3-2} follows from Hoeffding's inequality (Lemma~\ref{lemma:hoeffeding}).
By a union bound over all $e\in [n]$, we see that $\Pr[\xi_t] \ge 1-\frac{\delta}{2t^2}$. 
...
and Eq.~\eqref{eq:u-c-1-9} holds due to Eq.~\eqref{eq:rvtmp-5}.
...
Note that the exchange set $b$ defined here is different from the exchange set $b$ used in Case (1).
...
\label{eq:u-c-2-0}
\end{equation}
where the last inequality follows from the assumption that $\rad_t(e) < \frac{\Delta_e}{3\rank(\B)}$.
...
& =	\inn{\vec {\bar w}_t, \vec \chi_b} + \inn{\vec \rad_t, \vec\chi_b^2} + 2\inn{\vec \rad_t\odot \vec d, \vec \chi_b} \label{eq:cmr-2-1} \\
...
Eq.~\eqref{eq:cmr-2-1} follows from Lemma~\ref{lemma:vector-technical};
Eq.~\eqref{eq:u-c-2-3} follows from the assumption that $\xi_t$ occurs and Lemma~\ref{lemma:ci-property};
Eq.~\eqref{eq:u-c-2-3.5} follows from  H\"older's inequality;
...
Using the definition of $\rad_{t_e}$, we have
\begin{equation}
\frac{\Delta_e}{3\rank(\M)} \le 
R\sqrt{\frac{2\log\left(4n t_e^3/\delta\right)}{T(e)-1}} \le
R\sqrt{\frac{2\log\left(4n T^3/\delta\right)}{T(e)-1}}.
\label{eq:bound-te}
\end{equation}
By solving Eq.~\eqref{eq:bound-te} for $T(e)$, we obtain
\begin{equation}
\label{eq:bound-te-2}
T(e) \le \frac{18 \rank(\M)^2 R^2}{\Delta_e^2} \log(4nT^3/\delta)+1.
\end{equation}


%Notice that $T=\sum_{e\in[n]} T(e)$. 
%Hence the theorem follows by summing up Eq.~\eqref{eq:bound-te-2} for all $e\in [n]$ and solving for $T$.


Now we define $\mathbf{\tilde H}=\max\big\{\rank(\M)^2 R^2\mathbf{H},1\big\}$.
% and $T_* = 395\mathbf{\tilde H}\log\left(4n\mathbf{\tilde H}/\delta\right)+2n$.
In the rest of the proof, we show that 
\begin{equation}
\label{eq:cmr-t-1}
T \le 499\mathbf{\tilde H}\log\left(4n\mathbf{\tilde H}/\delta\right)+2n.
\end{equation}
Notice that the theorem follows immediately from Eq.~\eqref{eq:cmr-t-1}.
%Notice that the right-hand side of Eq.~\eqref{eq:cmr-t-1} 

If $n \ge \frac{1}{2}T$, then we see that $T \le 2n$ and therefore Eq.~\eqref{eq:cmr-t-1} holds immediately.
Next we assume that $n < \frac{1}{2}T$.
Since $T>n$, we can write 
\begin{equation} 
\label{eq:cmr-t-3}
T = C\mathbf{\tilde H}\log\left(4n\mathbf{\tilde H}/\delta\right) + n, \text{ for some }C>0.
\end{equation}
If $C \le 499$, then it is clear that Eq.~\eqref{eq:cmr-t-1} holds.
Suppose, on the contrary, that $C > 499$.
Notice that $T=\sum_{e\in[n]} T(e)$. 
By summing up Eq.~\eqref{eq:bound-te-2} for all $e\in[n]$, we have
\begin{align}
	T &\le n+\sum_{e\in [n]}\frac{18 \rank(\M)^2 R^2}{\Delta_e^2} \log(4nT^3/\delta) \nonumber\\
		&\le n+18 \mathbf{\tilde H}\log(4nT^3/\delta) \nonumber \\
		&= n+18 \mathbf{\tilde H}\log(4n/\delta)+ 54\mathbf{\tilde H}\log(T)\nonumber \\
		&\le n+18 \mathbf{\tilde H}\log(4n/\delta)+ 54\mathbf{\tilde H}\log\left(2C\mathbf{\tilde H}\log\left(4n\mathbf{\tilde H}/\delta\right)\right)\label{eq:cmr-t-2} \\
		&= n+18 \mathbf{\tilde H}\log(4n/\delta)+ 54\mathbf{\tilde H}\log\left(2C\right)
		+54\mathbf{\tilde H}\log(\mathbf{\tilde H})
		+54\mathbf{\tilde H}\log\log\left(4n\mathbf{\tilde H}/\delta\right) \nonumber \\
		&\le n+18 \mathbf{\tilde H}\log(4n\mathbf{\tilde H}/\delta)+ 54\mathbf{\tilde H}\log\left(2C\right)\log(4n\mathbf{\tilde H}/\delta)
		+54\mathbf{\tilde H}\log(4n\mathbf{\tilde H}/\delta)
		+54\mathbf{\tilde H}\log(4n\mathbf{\tilde H}/\delta)\label{eq:cmr-t-4}\\
		&=n+(126+54\log(2C))\mathbf{\tilde H}\log(4n\mathbf{\tilde H}/\delta)\nonumber \\
		&< n+C\mathbf{\tilde H}\log(4n\mathbf{\tilde H}/\delta) \label{eq:cmr-t-5}\\
		&= T,\label{eq:cmr-t-0}
\end{align}
where Eq.~\eqref{eq:cmr-t-2} follows from Eq.~\eqref{eq:cmr-t-3} and the assumption that $n<\frac{1}2 T$;
Eq.~\eqref{eq:cmr-t-4} follows from the fact that $\mathbf{\tilde H} \ge 1$ and $\delta < 1$;
Eq.~\eqref{eq:cmr-t-5} follows since $126+54\log(2C) < C$ for all $C > 499$;
and Eq.~\eqref{eq:cmr-t-0} is due to Eq.~\eqref{eq:cmr-t-3}.
Now we see that Eq.~\eqref{eq:cmr-t-0} is a contradiction. 
Therefore we obtain that $C\le 499$ and we have proved Eq.~\eqref{eq:cmr-t-1}.
...
Therefore, although its probability of error guarantee matches the parameter-free \AlgorithmBud algorithm up to logarithmic factors, this modified algorithm is considered weaker than \AlgorithmBud. 
...
In the following theorem, we show that \AlgorithmPAC is indeed an $(\epsilon,\delta)$-PAC algorithm and has a sample complexity similar to \Algorithm.
...
Moreover, the sample complexity of \AlgorithmPAC decreases when $\epsilon$ increases.
...
We shall assume that $\xi$ occurs in the rest of the proof.
...
Let $T(e)$ denote the total number of pulls of arm $e\in [n]$.
Let $t_e$ be the last round in which arm $e$ is pulled, i.e., $p_{t_e} = e$. 
...
T(e) \le R^2\min\left\{\frac{18 \rank(\B)^2}{\Delta_e^2},\frac{16K^2}{\epsilon^2}\right\} \log(4nT^3/\delta)+1.
\end{equation}
...
In the rest of the proof, we focus on claim \textbf{(a)}; the claim \textbf{(b)} can be
	 proven using an almost identical argument.
...
Since $\mathbb A$ is a $\delta$-correct algorithm and $\delta < 1/4$, we have $\Pr_0[\mathcal B]\ge 3/4$. Therefore, we have that the random event $\mathcal S=\mathcal A \cap \mathcal B \cap \mathcal C$ occurs with probability at least $1/4$ under $H_0$.
...
\textbf{Notations.}
Let $\vec w \in \RR^n$ be the vector of the expected rewards of arms.
...
Then, we show that, suppose arm $e$ has a gap $\Delta_e$ larger than the ``reference'' gap $\Delta_{(n-t+1)}$ of phase $t$, 
then arm $e$ must be correctly classified by $M_t$, i.e., $e\in M_t$ if and only if $e\in M_*$.
\begin{lemma}
Fix any phase $t>0$. 
...
It shows that if $\Delta_e \ge \Delta_{(n-t+1)}$ for some arm $e$, then the empirical gap of arm $e$,
	$\bar w_t(M_t) - \bar w_t(\tilde M_{t,e})$, is greater than $\frac23 \Delta_{(n-t+1})$.
\begin{lemma}
...
Recall the definition of $\tilde M_{t,e}$, which ensures that $e\in \tilde M_{t,e}$ if and only if $e\not\in M_t$.
...
The next lemma shows that, during phase $t$, if $\Delta_e \le \Delta_{(n-t+1)}$ for some arm $e$, then the empirical gap of arm $e$ is smaller than $\frac13 \Delta_{(n-t+1})$.
...
$e\in (b_+\cup b_-)$, $b_+\subseteq M_* \del M_t$, $b_- \subseteq M_t \del M_*$, $M_t \oplus b \in \M$ and $\inn{\vec w,\vec\chi_b}\ge \Delta_e > 0$.
...
\begin{proof}
First, we show that the algorithm takes at most $T$ samples.
...
Therefore, the total number of samples used by the algorithm is bounded by
\begin{align*}
\sum_{t=1}^n \tilde T_t 
...
\end{equation}
However Eq.~\eqref{eq-fdbg-contract} is contradictory to the definition of $p_t \triangleq \argmax_{e\in [n]\del (A_t\cup B_t)} \bar w_t(M_t)- \bar w_t(\tilde M_{t,e})$.
...
Due to the fact that $M,M'$ and $e$ are chosen arbitrarily, we have verified that $\B_{\Matroid(n)}$ is an exchange class for $\M_{\Matroid(T)}$.
...
Let $M$ and $M'$ be two different matchings of $G$.
...
We conduct experiments on a real-world dataset with decision classes corresponding to collections of spanning trees.
...
Recall that the sampling process of \AlgorithmBud is divided into $n$ phases and, in the end of each phase, the algorithm either accepts or rejects an arm.
In practice, for some decision classes, there may exists a phase $t<n$ such that %the set of accepted arms $A_t$ and the set of rejected arms $B_t$ may be large enough such that% 
there is only a unique set of arms which satisfies all constraints of phase $t$, i.e. $|\mathcal M_{A_t,B_t}|=1$ (see Section~\ref{section:csar} for the definitions of $A_t, B_t$ and $\mathcal M_{A_t,B_t}$).
By the design of \AlgorithmBud, one can see that this unique element of $\mathcal M_{A_t,B_t}$ will be the output of algorithm.
In this case, the remaining budget for phases $t+1,\ldots, n$ are wasted in the sense that they do not affect the output.
Therefore, in our experiment, we modify \AlgorithmBud to utilize these remaining budget in this case by using the following heuristic.
If $|\mathcal M_{A_t,B_t}|=1$ for some $t < n$, we stop the algorithm and then re-run the algorithm using the remaining budget.
During re-running the algorithm, the algorithm computes the empirical means of arms by using all samples including the samples from all previous runs of the algorithm. 
This process continues until the the algorithm terminates normally or the remaining budget is less than 30\% of the original budget.
%By recycling the samples from previous runs, our algorithm utilize the full budget.
%This heuristic improves 
%In the rest of this section, the \AlgorithmBud algorithm


\textbf{Evaluation method.} 
We use the following evaluation procedure to compare the sample efficiency among \Algorithm, \AlgorithmBud and \Uniform.
Since \AlgorithmBud and \Uniform are both learning algorithms in the fixed budget setting, the comparison among them is straightforward:
...
For each instance of ISP network, we run \Algorithm independently for 1000 times.
