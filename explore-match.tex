...
\For{$t=n,n+1,\ldots$}
	...
	\EndIf
	\State $D_t \gets \tilde M_t \ominus M_t$ \label{algo:step:D}
	\State $p_t \gets \argmax_{e \in D_t} \rad_t(e)$
	\State Play $p_t$ and observe the reward
	\State Update $T_{t+1}(p_t)$ and $\bar w_{t+1}(p_t)$
	\EndFor
\end{algorithmic}
\caption{CombPAC: Fixed confidence pure exploration algorithm}
\label{algo:pac}
\end{algorithm}
...
In addition, the number of samples $T$ used by the algorithm satisfy
$$
...
If Algorithm~\ref{algo:pac} stops on round $T$, then $w(M_*)-w(\out) \le \epsilon$.
\end{lemma}

\begin{proof}
Suppose that $\out \not= M_*$.
Since the algorithm stops on round $T$, we know that $\out = M_T$.
Consider the diff-set $D=M_* \ominus M_T$ and the diff-set $D_T$ defined in Step~\ref{algo:step:D} of Algorithm~\ref{algo:pac}.
...
\section{Benchmark: uniform algorithm}
